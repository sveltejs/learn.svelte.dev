<script>
	...
	$: is_svelte = /Partie (1|2)/.test($page.data.exercise.part.title);
</script>
		...
		<div class="error">
			<div>
				...
				{#if error.message === 'Your browser does not support all necessary features'}
					<p>
						Cette application nécessite des fonctionnalités web avancées. Merci d'utiliser un autre
						navigateur que Safari.
					</p>
				{:else if /firefox/i.test(navigator.userAgent)}
					<p>
						Nous n'avons pas pu lancer l'application. Merci de vous assurer que
						<a
							target="_blank"
							rel="noreferrer"
						...
						</a> sont autorisés pour ce site, et désactivez la Protection de Tracking Améliorée ("Enhanced
						Tracking Protection").
					</p>
					<p>
				...
				{:else if /chrome/i.test(navigator.userAgent) && !/edg/i.test(navigator.userAgent)}
					<p>
						Nous n'avons pas pu lancer l'application. Assurez-vous que les cookies d'applications
						tierces sont autorisés pour ce site — cliquez sur l'icône
						<svg
				...
				{:else}
					<p>
						Nous n'avons pas pu lancer l'application. Assurez-vous que les cookies d'applications
						tierces sont autorisés pour ce site.
					</p>
				{/if}

				{#if is_svelte}
					<a href="https://svelte.dev/tutorial/{$page.data.exercise.slug}">
						Aller plutôt à l'ancien tutoriel <Icon name="arrow-right" />
					</a>
				{/if}
			</div>
